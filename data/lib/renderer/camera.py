...
class Camera:
    ...
    def get_gl_matrix(self):
        ...
        model_view[2,3] = zz
        #model_view[2,3] += self.disz*100
        #model_view[0,3] += self.disx*100

        perspective = np.zeros_like(perspective)
        perspective[0,0] = 1.23   # 1.0625
        perspective[1,1] = 1.23 #2.1853125
        perspective[3,2] = -1
        ...
        perspective = np.zeros_like(perspective)
        perspective[0,0] = 2.1853125     # 1.0625
        perspective[1,1] = 2.1853125     # 1.23
        perspective[3,2] = -1
        perspective[2,2] = - (zFar + zNear) / (zFar - zNear)
        perspective[2,3] = -(2 * zFar * zNear) / (zFar - zNear)
        '''

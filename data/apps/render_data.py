...
def render_prt_ortho(out_path, folder_name, subject_name, shs, rndr, rndr_uv, im_size):
    ...
    mesh_file = os.path.join(folder_name, 'mesh.npz')
    if not os.path.exists(mesh_file):
        print('ERROR: obj file does not exist!!', mesh_file)
        return 
    
    prt_file = os.path.join(folder_name, 'PRT.npz')
    if not os.path.exists(prt_file):
        print('ERROR: prt file does not exist!!!', prt_file)
        return
    
    text_file = os.path.join(folder_name, 'texture.jpg')
    ...
    tmp = np.load(mesh_file)
    vertices = tmp["v"].astype(np.float64)*100

    faces = tmp["f"].astype(np.int64)
    tmp = np.load(mesh_file.replace("mesh","uv"))
    textures = tmp["vt"].astype(np.float64)
    face_textures = tmp["ft"].astype(np.int64)
    ...
    for iic in range(256):
        tmp = np.random.choice(shs.shape[0], 4, replace = False)
        for llight in range(4):
            y = iic/128*math.pi
            R = make_rotate(0, y, 0)
            rndr.rot_matrix = R
            ...
            out_all_f = out_all_f * 255
            out_all_f.astype(np.uint8)
            cv2.imwrite(os.path.join(out_path, '%03d_%03d.png'%(iic,llight)),out_all_f)


if __name__ == '__main__':
    shs = np.load('../lib/renderer/env_sh.npy')
    ...
    parser.add_argument('-f', '--fro',  type=int, default=0)
    ...
    render_prt_ortho(args.out_dir, args.input, args.input, shs, rndr, rndr_uv, args.size)
